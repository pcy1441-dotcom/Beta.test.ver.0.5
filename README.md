...
<style>
  ...
  /* --- 능력별 특수 색상 --- */
  .skill-gunslinger { background: linear-gradient(45deg, #FF1801 40%, #3712FF 60%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; display: inline-block; }
  .skill-yellow { color: #D4AF37; text-shadow: 1px 1px 0px #fff; } 
  ...
  .info-btn { padding: 6px 12px; border-radius: 6px; border: 1px solid #444; background: #222; color: #888; cursor: pointer; font-size: 11px; margin: 4px; }
...
<div id="game-container">
  <h1 style="color:#333; letter-spacing:8px;">MURDER BETA</h1>
  ...
  <div id="menu">
    ...
    <button onclick="startGame()" style="margin-top:20px; padding:15px 40px; border-radius:12px; border:none; background:#222; color:#fff; font-weight:bold; cursor:pointer;">GAME START</button>
  </div>

  <div id="draw-area" style="display:none;">
    <div id="playerNum" style="font-size: 28px; margin-bottom: 20px; color: #666;"></div>
    <button onclick="draw()" style="background:var(--accent-red); padding:15px 40px; border-radius:12px; border:none; color:#fff; font-weight:bold; cursor:pointer;">역할 확인</button>
  </div>
  ...
  <div id="result">
    ...
    <div id="skill"></div>
    <div id="desc"></div>
    <button onclick="nextPlayer()" style="background:#000; color:#fff; width:100%; padding:15px; border-radius:10px; border:none; cursor:pointer;">확인 완료</button>
  </div>
  ...
  <div id="final-screen">
    ...
    <br><br>
    <button onclick="location.reload()" style="background:none; border:1px solid #333; color:#333; padding:10px 20px; cursor:pointer;">새 게임 시작</button>
  </div>
</div>
...
function startGame() {
  ...
  // 시민 중 능력 받을 2명 미리 지정
  let citizenIndices = [];
  roles.forEach((role, idx) => { if(role === "시민") citizenIndices.push(idx); });
  citizenIndices.sort(() => Math.random() - 0.5);
  let luckyCitizens = citizenIndices.slice(0, 2); // 최대 2명

  // 각 플레이어별 능력 미리 할당
  assignedSkills = roles.map((role, idx) => {
    if (!useSkills) return { name: "무능력", desc: "능력이 없습니다." };
    
    if (role === "머더") return null; // draw에서 타입에 따라 결정

    if (role === "시민") {
      if (luckyCitizens.includes(idx)) {
        // 시민 능력 중 무능력 제외하고 셔플해서 할당
        let citizenSkillList = skills.시민.sort(() => Math.random() - 0.5);
        return citizenSkillList[luckyCitizens.indexOf(idx) % citizenSkillList.length];
      }
      return { name: "무능력", desc: "능력이 없습니다." };
    }

    // 보안관/의사: 해당 리스트에서 랜덤 1개
    let list = skills[role];
...
function draw() {
  ...
  let chosen = assignedSkills[current];

  if (useSkills && role === "머더") {
    if (roundMurderType.name === "Standard") {
      chosen = skills.머더[Math.floor(Math.random() * skills.머더.length)];
    } else {
      chosen = { name: "SPECIAL TYPE", desc: "특수 타입입니다. 정체는 마지막에 공개됩니다." };
    }
  }
...
function nextPlayer() {
  ...
  if (current >= totalPlayers) {
    ...
    } else { alert("게임 시작!"); location.reload(); }
    return;
  }
  document.getElementById("result").style.display = "none";
  document.getElementById("draw-area").style.display = "block";
